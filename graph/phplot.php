...
class PHPlot
{
    ...
    static protected $datatypes = array(
        // See DecodeDataType() and $datatype_* flags
    ...
    protected function GetDefaultTTFont()
    {
        if (!isset($this->default_ttfont)) {
            ...
            $fonts = array(
                ...
                // For Linux with a correct GD font search path
                'Verdana.ttf',
                'Arial.ttf',
                'Helvetica.ttf',
                // For Windows, maybe others
                'liberation/LiberationSans-Regular.ttf',
                // For newer Ubuntu etc
                'ttf-liberation/LiberationSans-Regular.ttf',
                // For older Debian, Ubuntu, etc
                'benjamingothic.ttf',
    ...
    function DrawMessage($text, $options = NULL)
    {
        ...
        extract(array_merge(
            array(
                ...
                // Draw image background per SetBgImage(), SetBackgroundColor()
                'draw_border' => FALSE,
                // Draw image border as set with SetBorder*()
                'force_print' => TRUE,
                // Ignore SetPrintImage() setting and always output
                'reset_font' => TRUE,
                // Reset fonts (to avoid possible TTF error)
                'text_color' => '',
                // If not empty, text color specification
                'text_wrap' => TRUE,
                // Wrap the message text with wordwrap()
                'wrap_width' => 75,
    ...
    protected function GetLegendPosition($width, $height)
    {
        ...
        switch ($mode) {
            ...
            case 'plot': // SetLegendPosition with mode='plot', relative coordinates over plot area.
                return array(
                    (int) ($x_base * $this->plot_area_width - $x * $width)
